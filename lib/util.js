...
util = {
    ...
    /**
     * Indicates if the given location represents a resource from Postman-API
     *
    ...
    isRemote: function (location) {
        return _.isString(location) && !fs.existsSync(location) &&
            (POSTMAN_API_URL_REGEX.test(location) || ID_REGEX.test(location));
    },
    ...
    fetchJson: function (type, location, options, callback) {
        ...
        if (!fs.existsSync(location) && POSTMAN_API_PATH_MAP[type] && postmanApiKey && ID_REGEX.test(location)) {
            location = `${POSTMAN_API_URL}/${POSTMAN_API_PATH_MAP[type]}/${location}`;
        }

        // if the Postman-API-URL does not contain a query-param for API Key, add it as a header
        if (postmanApiKey && POSTMAN_API_URL_REGEX.test(location) && !(/.\?apikey=./).test(location)) {
