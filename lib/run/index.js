    ...
    getOptions(options, function (err, options) {
        ...
        runner.run(options.collection, {
            ...
            _.isArray(reporters) && _.forEach(reporters, function (reporterName) {
                ...
                try {
                    // exclude private variables and postmanApiKey from options passed to the reporter
                    let reporterOptions = _.omitBy(options, (_value, key) => {
                        return key.startsWith('_') || key === 'postmanApiKey';
                    });
                    ...
                    Reporter && (emitter.reporters[reporterName] = new Reporter(emitter,
                        _.get(options, ['reporter', reporterName], {}), reporterOptions));
                }
                catch (error) {
