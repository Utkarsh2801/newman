...
module.exports = function (options, callback) {
    ...
    var emitter = new EventEmitter(), // @todo: create a new inherited constructor
        ...
        originalEnvironmentArg = options.environment, // used during environment-sync
        stopOnFailure,
        entrypoint;
                 ...
                 */
                    ...
                    async.parallel([
                        ...
                        (next) => {
                            ...
                            let { environment } = emitter.summary,
                                location;

                            // if the argument provided is a URL and contains apikey query-param, use it directly
                            // else use the postman-id from the environment-object
                            // this is done to not miss any auth-details
                            location = (/^https?:\/\/.*apikey=.*/).test(originalEnvironmentArg) ?
                                originalEnvironmentArg : environment.id;

                            util.syncJson(location, environment,
                                { postmanApiKey: options.postmanApiKey, type: 'environment' },
                                (err) => {
                                    if (err) {
