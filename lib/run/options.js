var _ = require('lodash'),
    ...
    /**
     * The message displayed when the data related to the api-key-alias is not found
     *
     * @const
     * @type {String}
     */
     ...
     * @param {String} options.postmanApiKeyAlias - The alias of the API Key to be loaded
     ...
     * @returns {*}
     */
    postmanApiKeyLoader = async function ({ postmanApiKeyAlias, _login: login }, cb) {
        let aliasDetails;

        login && login._profiles && ([aliasDetails] = _.filter(login._profiles, ['alias', postmanApiKeyAlias]));

        if (!aliasDetails) {
            return cb(NO_AUTHORIZATION_DATA);
        }
    ...
    externalLoader = function (type, location, options, cb) {
        async.waterfall([
            (next) => {
                ...
                if (!options.postmanApiKey && options.postmanApiKeyAlias &&
                    util.isRemote(location) && !(/.\?apikey=./).test(location)) {
                    return postmanApiKeyLoader(options, next);
                }
            ...
            (apikey, next) => {
                ...
                if (_.isString(location)) {
                    return util.fetchJson(type, location, options, next);
                }

                // if the location is an object
...
module.exports = function (options, callback) {
    ...
    let commonOptions = _.pick(options, ['postmanApiKey', 'postmanApiKeyAlias', '_login']);
